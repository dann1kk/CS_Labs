...
def sha256_sum(message):
    # Constant for modulo 2 addition operation **32
    MOD32 = 2**32
    # Variable initialization (first 32 bits of the fractional parts of the square roots of the first eight primes [2 to 19]):
    h0 = 0x6A09E667
    h1 = 0xBB67AE85
    h2 = 0x3C6EF372
    h3 = 0xA54FF53A
    ...
    # Constant table (first 32 bits of the fractional cube roots of the first 64 primes [2 to 311]):
    ...
    # Pre-processing (addition of the original message):
    m_len = len(message)
    p_len = 64 - m_len % 64 - 8
    m = message + (0b10000000).to_bytes(p_len, byteorder='little', signed=False) + (m_len * 8).to_bytes(8, byteorder='big', signed=False)

    # Next, the message is processed in successive portions of 512 bits:
    block = []

    # Break the message into blocks of 512 bits
    for j in range(len(m) // 64):
        block.append(m[j*64:(j+1)*64])
        # Each block is divided into 16 words 32 bits long (with byte order from high to low byte inside the word):
